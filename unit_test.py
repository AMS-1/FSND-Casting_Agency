...
# database_path='postgres://postgres:1234@localhost:5432/casting_test_fsnd'
# (incase of locally run)
...
class Casting_Agency_TestCase(unittest.TestCase):
    ...
    def test_add_new_movie_valid_authorized(self):
        ...
        data = json.loads(resp.data)
        print(
            'test create movie with valid auth----------------->',
            data.items)
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(data['success'])
    ...
    def test_add_Actor_valid_authorized(self):
        ...
        data = json.loads(resp.data)
        print(
            "test create actor valid authorized ------------------------->",
            data.items)
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(data['success'])
